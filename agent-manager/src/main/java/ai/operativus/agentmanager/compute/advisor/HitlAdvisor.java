...
public class HitlAdvisor implements CallAdvisor, StreamAdvisor {
    ...
    private final ai.operativus.agentmanager.core.registry.ApprovalOperations approvalService;
    ...
    public HitlAdvisor(@Lazy ai.operativus.agentmanager.core.registry.ApprovalOperations approvalService,
    ...
    public void requireApprovalForTool(String toolName, String serializedArgs, String runId, String sessionId, String agentId) {
        ...
        ai.operativus.agentmanager.core.model.DecisionPackage.DecisionTier tier;
        Optional<DecisionPackage.DecisionTier> resolved = resolveTierFromProviders(toolName);
        if (resolved.isPresent()) {
            tier = resolved.get();
        } else if (DESTRUCTIVE_TOOLS.contains(toolName)) {
            tier = ai.operativus.agentmanager.core.model.DecisionPackage.DecisionTier.TIER_3_DESTRUCTIVE;
        } else if (finopsGateEnabled && finopsTools.contains(toolName)) {
            tier = ai.operativus.agentmanager.core.model.DecisionPackage.DecisionTier.TIER_2_FINOPS_BLOCK;
        } else {
            tier = ai.operativus.agentmanager.core.model.DecisionPackage.DecisionTier.TIER_1_SAFE;
        }
