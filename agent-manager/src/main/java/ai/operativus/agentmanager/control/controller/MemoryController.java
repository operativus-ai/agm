...
import ai.operativus.agentmanager.control.service.queue.MemoryOptimizationJobHandler;
import ai.operativus.agentmanager.core.model.AddMemoryRequest;
...
public class MemoryController {
    ...
    public ResponseEntity<Map<String, String>> optimizeMemories(@RequestParam(required = false) String userId) throws Exception {
        String uid = ai.operativus.agentmanager.control.config.SecurityContextUtils.resolveUserId(userId);
    ...
    private static String resolvePermittedUserId(String explicitUserId) {
        if (isCurrentUserAdmin()) {
            return ai.operativus.agentmanager.control.config.SecurityContextUtils.resolveUserId(explicitUserId);
        }
        String boundUserId = currentUserId();
        return boundUserId != null
                ? boundUserId
                : ai.operativus.agentmanager.control.config.SecurityContextUtils.resolveUserId(null);
    }
