...
import ai.operativus.agentmanager.core.model.enums.AuditActionType;
import ai.operativus.agentmanager.core.registry.AgentAdminOperations;
...
public class AgentAdminService implements AgentAdminOperations {
    ...
    public AgentDefinition updateAgent(String id, AgentDefinition dto) {
        ...
        entity.setComplianceTier(dto.complianceTier() != null ? dto.complianceTier() : ai.operativus.agentmanager.core.entity.ComplianceTier.TIER_1_STANDARD);
    ...
    @Transactional(readOnly = true)
    @PreAuthorize("hasPermission(#id, 'AgentDefinition', 'read')")
    public ai.operativus.agentmanager.core.model.TopologyDTO getAgentTopology(String id) {
        AgentEntity entity = agentRepository.findByIdAndOrgId(id, callerOrgId())
                .orElseThrow(() -> new ResourceNotFoundException("Agent", id));

        List<ai.operativus.agentmanager.core.model.TopologyDTO.Node> nodes = new java.util.ArrayList<>();
        List<ai.operativus.agentmanager.core.model.TopologyDTO.Edge> edges = new java.util.ArrayList<>();
        
        // Root node
        nodes.add(new ai.operativus.agentmanager.core.model.TopologyDTO.Node(entity.getId(), entity.getName(), "agent"));
        
        // Tools
        if (entity.getTools() != null) {
            for (String tool : entity.getTools()) {
                String toolNodeId = "tool_" + tool;
                nodes.add(new ai.operativus.agentmanager.core.model.TopologyDTO.Node(toolNodeId, tool, "tool"));
                edges.add(new ai.operativus.agentmanager.core.model.TopologyDTO.Edge(entity.getId() + "_" + toolNodeId, entity.getId(), toolNodeId));
            }
        }
        ...
        if (entity.isTeam() && entity.getMembers() != null) {
            for (String memberId : entity.getMembers()) {
                agentRepository.findById(memberId).ifPresent(member -> {
                    nodes.add(new ai.operativus.agentmanager.core.model.TopologyDTO.Node(member.getId(), member.getName(), "member"));
                    edges.add(new ai.operativus.agentmanager.core.model.TopologyDTO.Edge(entity.getId() + "_" + member.getId(), entity.getId(), member.getId()));
                });
            }
        }
        
        // Transition edges (DAG constraints)
        List<ai.operativus.agentmanager.core.model.TopologyDTO.TransitionConstraint> transitionConstraints =
            transitionEdgeRepository.findByTeamId(id).stream()
                .map(edge -> new ai.operativus.agentmanager.core.model.TopologyDTO.TransitionConstraint(
                        edge.getId(), edge.getSourceAgentId(), edge.getTargetAgentId()))
                .collect(java.util.stream.Collectors.toList());
        
        return new ai.operativus.agentmanager.core.model.TopologyDTO(nodes, edges, transitionConstraints);
    }
    ...
    @Override
    @Transactional
    public ai.operativus.agentmanager.core.model.definitions.AgentDefinition rollbackAgent(String agentId, String auditId) {
        ...
        try {
            ai.operativus.agentmanager.core.model.definitions.AgentDefinition restoredDef =
                    objectMapper.readValue(snapshot.getChangeset(), ai.operativus.agentmanager.core.model.definitions.AgentDefinition.class);
            ai.operativus.agentmanager.core.model.definitions.AgentDefinition result = updateAgent(agentId, restoredDef);
    ...
    private AgentEntity mapFromDefinition(AgentDefinition definition) {
        ...
        entity.setComplianceTier(definition.complianceTier() != null ? definition.complianceTier() : ai.operativus.agentmanager.core.entity.ComplianceTier.TIER_1_STANDARD);
    ...
    public void cancelRun(String runId) {
        ...
        ai.operativus.agentmanager.core.registry.RunOperations runOps = runRepository;
