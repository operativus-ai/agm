...
public class TeamOrchestrationTest {
    ...
    private ai.operativus.agentmanager.compute.service.AgentService agentService;

    @org.springframework.beans.factory.annotation.Autowired
    private ai.operativus.agentmanager.core.model.definitions.AgentRegistry agentRegistry;

    @MockitoBean
    private ai.operativus.agentmanager.control.service.SessionService sessionService;

    @MockitoBean
    private ai.operativus.agentmanager.control.service.SettingsService settingsService;

    @MockitoBean
    private ai.operativus.agentmanager.control.repository.RunRepository runRepository;
    
    @MockitoBean
    private org.springframework.ai.vectorstore.VectorStore vectorStore;
    
    @MockitoBean
    private ai.operativus.agentmanager.control.service.PersistentChatMemory persistentChatMemory;
    
    // We mock DelegationTool to avoid circular deps if any, or just let it be loaded if using availableTools
    // @MockBean
    // private ai.operativus.agentmanager.compute.tools.DelegationTool delegationTool;
